...
class SrcDatastoreGenerator:
    def __init__(self, context, raw_entity_name):
        # type: (SrcDatastoreGenerator, Context, str) -> None
        C = context
        self.context = context

        base_store_name = f'Base{raw_entity_name}Store'
        store_name = f'Src{raw_entity_name}Store'
        repo_name = f'Src{raw_entity_name}Repository'
        file_path = f"{C.ROOT_MODULE_DIR}/migrational/datastores/src/{store_name}.java"

        preamble = StringBuilder().add_lines(
            f'package {C.ROOT_MODULE_FQN}.migrational.datastores.src;',
            f'',
            ...
            f'import {C.ROOT_MODULE_FQN}.repository.src.{repo_name};',
        ).build()
